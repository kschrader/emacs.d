...
(setq user-full-name "Kurt Schrader")
(setq user-mail-address "kschrader@gmail.com")

(setenv "PATH" (concat "/usr/local/bin:/Users/kschrader/.cabal/bin:" (getenv "PATH")))
(require 'cl)
...
(defun kschrader/packages-installed-p ()
  (loop for pkg in kschrader/packages
        when (not (package-installed-p pkg)) do (return nil)
        finally (return t)))

(unless (kschrader/packages-installed-p)
  (message "%s" "Refreshing package database...")
  (package-refresh-contents)
  (dolist (pkg kschrader/packages)
...
(defvar kschrader/vendor-dir (expand-file-name "vendor" user-emacs-directory))
(add-to-list 'load-path kschrader/vendor-dir)

(dolist (project (directory-files kschrader/vendor-dir t "\\w+"))
...
(defun kschrader/engage-lisp-power ()
  (lisp-power-mode t))

(dolist (mode lisp-modes)
  (add-hook (intern (format "%s-hook" mode))
            #'kschrader/engage-lisp-power))
...
(setq markdown-css-path (expand-file-name "markdown.css" kschrader/vendor-dir))
